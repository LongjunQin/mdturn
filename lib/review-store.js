...
const APPLY_HINT = '本文件是对 sourceFile 的评审批注(由 md-read 生成,与同目录同名 .md 一一对应)。' +
  '改稿协议:只处理 status=open 的;用 quote+headingPath 定位(lineStart 仅辅助,改稿后可能偏移);' +
  '改完把该条标 status=applied + appliedAt + appliedBy + appliedNote;绝不重做 status=applied 的。';
...
function getDataDir(options = {}) {
  return path.resolve(options.dataDir || process.env.MDREAD_DATA_DIR || path.join(PROJECT_DIR, '.mdread'));
}
...
function getPaths(options = {}) {
  ...
  return {
    dataDir,
    registry: path.join(dataDir, 'registry.json'),
    reviews: path.join(dataDir, 'reviews.json'),
    port: path.join(dataDir, 'port'),
    url: path.join(dataDir, 'url'),
    owner: path.join(dataDir, 'owner'),
  };
}
...
function ensureDataDir(options = {}) {
  ...
  for (const filePath of [paths.registry, paths.reviews, paths.port, paths.url, paths.owner]) {
...
function readAnnotations(absMd) {
  ...
  return {
    ...
    _apply: existing._apply || APPLY_HINT,
...
async function openReview(inputPath, options = {}) {
  ...
  return withReviewTransaction(absPath, async () => {
    ...
    const result = await mutateReviews((data) => {
      ...
      if (active) {
        if (sourceHash !== active.sourceHash) {
      ...
      return { review: { ...review }, reused: false };
    }, options);

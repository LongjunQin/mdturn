...
本目录是 **md-read**：提供电脑本地冻结审阅和手机远程批注。两种入口共用原文件旁的
...
## 任务一：在电脑上用 MDTurn 本地审阅

当用户说“电脑打开 / 本地审阅 / 在本机批注”时：

```bash
mdreview open "<文档绝对路径.md>"
```

- `mdreview open` 优先把 Markdown 打开到 MDTurn 的同一窗口多文档标签页；App 不可用时才回退本机浏览器，不经过公网。
- 命令输出的 localhost URL 是恢复入口，不是手机链接；手机或他人审阅必须使用 `mdshare`。
- 阅读与批注处于同一个冻结模式；只有提交审阅后才能进入独立编辑模式。
- `mdreview open` 会创建或复用审阅会话并冻结该文档；其他文档不受影响。
- “发我看 / 我要批注”但未说明终端时，先确认是电脑本地还是手机远程。
...
## 任务二：把某篇文档发到用户手机（生成链接）

当用户明确说“发我手机 / 生成分享链接 / 给别人看”时：

```bash
node "$TOOL/mdshare.js" "<该文档的绝对路径.md>"
```
- 把命令**输出的最后一行链接**原样发给用户即可。
- 给**别人**看(如同事)并要区分批注者:加 `--for 姓名`,如 `--for 小王`;可加 `--days 3` 设有效期。
- 若输出里出现“⚠️ 未检测到外网隧道”：说明 launchd 服务或 Tunnel 尚未就绪，先稍等并重试；必要时才使用 `serve.command` 手工启动。

只接受 `.md` 文件。同一文件和署名会复用原 ID/token；Quick Tunnel 重启后公网域名仍可能变化，应重新运行 `mdshare` 获取当前完整链接。

---

## 任务三：根据用户批注修改文档（关键：别重复做已完成的）

当用户说"按批注改一下这篇 / 根据我手机上的批注修改"时,**严格按下面协议**:
